...
import { getTitleById, updateTitle, getAllCategories } from '@/lib/firebase/firestore';
import { Category, Title } from '@/lib/types/models';
...
export default function EditTitlePage() {
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        ...
        base_price: titleData.base_price,
        price_tier: titleData.price_tier,
        is_official: titleData.is_official,
        status: titleData.status,
        purchasable_limit: titleData.purchasable_limit,
        purchased_count: titleData.purchased_count,
      });
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateTitle(titleId, formData);
      alert('肩書きを更新しました');
      router.push('/admin/titles');
    } catch (error) {
      console.error('更新エラー:', error);
      setError('肩書きの更新に失敗しました');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex justify-center items-center">
        <div className="text-center">
          ...
          <Link href="/admin/titles" className="text-indigo-600 hover:underline">
            ← 肩書き一覧に戻る
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">肩書きを編集</h1>
        </div>
          ...
          <div className="mb-8">
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-md">
              <p className="text-sm text-gray-600">
                購入済み枠数: <span className="font-semibold">{formData.purchased_count}</span> / {formData.purchasable_limit}
              </p>
            </div>
          </div>
